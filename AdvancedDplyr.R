...
corrrelation<- galton%>% group_by(pair)%>%summarize(cor(parentHeight, childHeight))
corrrelation


lm<- galton%>% group_by(pair)%>%do(tidy(lm(childHeight ~ parentHeight, data = .), conf.int = TRUE))
